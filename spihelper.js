...
importStylesheet( 'User:Sohom_Datta/spihelper.css' )
// --------------------------------------------------------------------------------------
// Copied from [[User:Timotheus_Canens/displaymessage.js]]
/**
...
async function spiHelperGenerateForm() {
	...
	if ( spiHelperActionsSelected.Case_act ) {
		...
		$( '#spiHelper_CaseAction', $actionView ).on( 'change', ( e ) => {
	...
	if ( spiHelperActionsSelected.Link ) {
		...
		for ( let i = 1; i <= spiHelperLinkTableUserCount; i++ ) {
			...
			if ( $( '#spiHelper_link_interactionTimeline' + i, $actionView ).prop( 'checked' ) ) {
				spiHelperUsersForLinks.interactionTimeline.push( username )
			}
			if ( $( '#spiHelper_link_timecardSPITools' + i, $actionView ).prop( 'checked' ) ) {
				spiHelperUsersForLinks.timecardSPITools.push( username )
			}
			if ( $( '#spiHelper_link_consolidatedTimelineSPITools' + i, $actionView ).prop( 'checked' ) ) {
				spiHelperUsersForLinks.consolidatedTimelineSPITools.push( username )
			}
			if ( $( '#spiHelper_link_pagesSPITools' + i, $actionView ).prop( 'checked' ) ) {
				spiHelperUsersForLinks.pagesSPITools.push( username )
			}
			if ( $( '#spiHelper_link_checkUserWikiSearch' + i, $actionView ).prop( 'checked' ) ) {
				spiHelperUsersForLinks.checkUserWikiSearch.push( username )
			}
		}

		const $linkViewList = $( '#linkViewResultsList', document )
		for ( const link in spiHelperUsersForLinks ) {
			if ( spiHelperUsersForLinks[ link ].length === 0 ) {
				continue
			}
...
async function spiHelperLog( logString ) {
	...
	if ( !logPageText.match( dateHeaderRe ) ) {
		if ( spiHelperSettings.reversed_log ) {
			...
			logPageText = logPageText.slice( 0, Math.max( 0, firstHeaderMatch.index ) ) + '== ' + dateString + ' ==\n' + logPageText.slice( Math.max( 0, firstHeaderMatch.index ) )
		} else {
			logPageText += '\n== ' + dateString + ' =='
		}
	}
	if ( spiHelperSettings.reversed_log ) {
		const firstHeaderMatch = logPageText.match( dateHeaderReWithAnyDate )
		logPageText = logPageText.slice( 0, Math.max( 0, firstHeaderMatch.index + firstHeaderMatch[ 0 ].length ) ) + '\n' + logString + logPageText.slice( Math.max( 0, firstHeaderMatch.index + firstHeaderMatch[ 0 ].length ) )
	} else {
		logPageText += '\n' + logString
	}
	...
	while ( pagesToCheck.length !== 0 ) {
		...
		for ( let i = 0; i < backlinks.length; i++ ) {
			if ( ( await spiHelperParseArchiveNotice( backlinks[ i ].title ) ).username === currentPageToCheck.replace( /Wikipedia:Sockpuppet investigations\//g, '' ) ) {
				...
				if ( pagesChecked.includes( backlinks[ i ].title ) ) {
...
async function spiHelperArchiveCase() {
	...
	const textToArchiveLines = []
	const textToKeepLines = []
	let sectionLines = []
...
async function spiHelperArchiveCaseSection( sectionId ) {
	...
	const newarchivetext = sectionText.slice( Math.max( 0, sectionText.search( spiHelperSectionRegex ) ) )
	let archivetext = await spiHelperGetPageText( spiHelperGetArchiveName(), true )
...
async function spiHelperMoveCase( target ) {
	...
	if ( targetPageText ) {
		...
		siteProtectionInformation.types.forEach( ( type ) => {
			let oldPageNameEntry = oldPageNameProtection.filter( ( dict ) => dict.type === type )
			let newPageNameEntry = newPageNameProtection.filter( ( dict ) => dict.type === type )
...
async function spiHelperGetSPIBacklinks( casePageName ) {
	...
	try {
		...
		return response.query.backlinks.filter( ( dictEntry ) => dictEntry.title.startsWith( 'Wikipedia:Sockpuppet investigations/' ) && !dictEntry.title.startsWith( 'Wikipedia:Sockpuppet investigations/SPI/' ) && !dictEntry.title.match( 'Wikipedia:Sockpuppet investigations/.*/Archive.*' ) )
	} catch ( error ) {
		return []
	}
}
		...
		if ( typeof spiHelperCustomOpts !== 'undefined' ) {
			...
			for ( let index = 0; index < keys.length; index++ ) {
				...
				if ( k in spiHelperValidSettings ) {
					if ( !spiHelperValidSettings[ k ].includes( v ) ) {
